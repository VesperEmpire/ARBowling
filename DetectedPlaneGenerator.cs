    ...
    public class DetectedPlaneGenerator : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < m_NewPlanes.Count; i++)
            {
                ...
                //MeshCollider has to be added dynamiclly. Otherwise the collider will not wrap the whole plane correctly 
                planeObject.AddComponent<MeshCollider>();
                planeObject.GetComponent<MeshCollider>().convex = true;
                
            }

            /*Before the start button is clicked. 
             We dynamically enable MeshCollider to wrap the entire plane by constantly switching the convex option*/
            if (GameObject.Find("DetectedPlaneVisualizer(Clone)")) {
                //if (StartButton.activeSelf == false)
                //{
                //    planeObject.GetComponent<MeshCollider>().convex = false;
                //    planeObject.GetComponent<MeshCollider>().convex = true;
                //}
                if (bowling.GetComponent<Rigidbody>().useGravity == false) {
                    planeObject.GetComponent<MeshCollider>().convex = false;
                    planeObject.GetComponent<MeshCollider>().convex = true;
                }
            }
